<?php
/*
 * Classroom Students Endpoint for SCMS
 * Handles fetching students for a specific classroom
 * 
 * File location: /scms_new_backup/index.php/api/teacher/classroom/{class_code}/students
 */
...
try {
    ...
    // First, try to get students from classroom_enrollments table
    try {
        $stmt = $pdo->prepare("
            SELECT 
                u.id as user_id,
        ...
        error_log('Classroom enrollments query failed: ' . $e->getMessage());
    }
    ...
    echo json_encode([
        'status' => true,
        'message' => 'All students fetched successfully (fallback)',
    ...
    echo json_encode([
        'status' => false,
        'message' => 'Error fetching classroom students: ' . $e->getMessage(),
        'data' => null
    ]);
}
